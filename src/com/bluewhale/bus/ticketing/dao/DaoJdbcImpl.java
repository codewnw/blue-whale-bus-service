...
import com.bus.ticketing.jdbc.mysql.dbUtil.DbUtil;
...
public class DaoJdbcImpl implements DaoJdbc {
	...
	public void create(Customer customer) throws IllegalArgumentException, DAOException {
		...
		Object[] values = { customer.getUserName(), customer.getPassword(), customer.getFirstName(),
				customer.getLastName(), customer.getPhoneNo(), customer.getDob(), customer.getEmail(),
				customer.getAddress(), customer.getGender(), customer.getCountry() };

		Connection connection;
		try {
			connection = MyDbUtil.getDbConnection();
			PreparedStatement statement = MyDbUtil.prepareStatement(connection, SQL_INSERT_NEW_customer, true, values);
			...
			try (ResultSet generatedKeys = statement.getGeneratedKeys()) {
				if (generatedKeys.next()) {
					customer.setcId(generatedKeys.getInt(1));
				} else {
	...
	public boolean find(String username, String password) throws DAOException {
        ...
        try (
            Connection connection = MyDbUtil.getDbConnection();
            PreparedStatement statement = MyDbUtil.prepareStatement(connection, SQL_Find_by_UserName_Password, false, values);
	...
	public boolean bookSeat(Booking booking, int customerId) {
		...
		Object[] values= {booking.getBookingStatus(), booking.getBusId(), booking.getSeatNo(), booking.getBookingPrice(),
				booking.getPaymentMode(), customerId, booking.getBookingCreatedDate(),booking.getFromPlace(), 
				booking.getToPlace(), booking.getTravelDate()};
		Connection connection;
		try {
			connection = MyDbUtil.getDbConnection();
			PreparedStatement statement = MyDbUtil.prepareStatement(connection, SQL_INSERT_NEW_seat_booking, true, values);
			int affectedRows = statement.executeUpdate();
			if (affectedRows == 0) {
	...
	public Customer findCustomer(String username) throws DAOException {
        ...
        try (
            Connection connection = MyDbUtil.getDbConnection();
            PreparedStatement statement = MyDbUtil.prepareStatement(connection, SQL_Find_Customer, false, values);
            ResultSet resultSet = statement.executeQuery();
        ) {
        	 if (resultSet.next()) {
	...
	public List<BookingHistory> findBookingHistoryByCustomer(int custId){
        ...
        try (
            Connection connection = MyDbUtil.getDbConnection();
            PreparedStatement statement = MyDbUtil.prepareStatement(connection, SQL_Booking_History_By_CustomerId, false, values);
            ResultSet resultSet = statement.executeQuery();
